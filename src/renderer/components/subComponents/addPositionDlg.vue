<template>
	<el-dialog :close-on-click-modal="false" :close-on-press-escape="false"  class="shortHeightDlg houseAddDlg" title="添加职位" v-model="isDlgVisable" >
	    <el-form :model="ruleForm" label-position="left" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
	    	<el-form-item label="职位名称" prop="positionName">
			  	<el-input v-model="ruleForm.positionName" placeholder="职位"></el-input>
			</el-form-item>
	  </el-form>
	...
	export default {
		...
		data: function() {
			return {
		        ...
		        rules: {
		        	positionName: validation.requiredBlur
		        }
			}
		},
		methods: {
	      ...
	      submitForm(formName) {
	        ...
	        _this.$refs[formName].validate((valid) => {
	          if (valid) {
	            _this.showLoading();
	            _this.$api.post(constants.KCURLS.CREATEPOSITION, _this.ruleForm,
		          function(res) {
		          	_this.hideLoading();
		          	if (res.succeeded ) {
		          		_this.$message({
