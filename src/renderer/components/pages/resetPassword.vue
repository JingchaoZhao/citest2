...
<div>
  ...
  <div class="loginDiv">
    <el-col :span="10" class="logoArea">
      ...
      <span>蜂鸟屋后台管理系统</span>
      <div></div>
      <img style="margin-top: 70px; width: 90px;" src="../../assets/logo_login.png" />
    ...
    <el-col :span="14" class="formArea">
      ...
      <mu-text-field class="userPwdInput" type="password" label="新密码" :fullWidth="true" v-model="password" :errorText="pwdErrorInfo" v-on:focus="resetPwdError()" v-on:blur="testPwd()" labelFloat/>
      <mu-text-field class="userPwdInput" type="password" label="确认新密码" :fullWidth="true" v-model="rpassword" :errorText="rpwdErrorInfo" v-on:focus="resetRPwdError()" v-on:blur="testRPwd()" labelFloat/>
      <div class="blockBtn greenBtn resetBtn" @click="handleSubmit">确 定</div>
      <div style="margin-top:10px">
        <img style="width:24px;height:24px" src="../../assets/go.png" />
        <router-link style="color:#07bf96;" to="/login">返回登录</router-link>
      </div>
    </el-col>
  </div>
</div>
    ...
    handleSubmit(ev) {
      ...
      that.$api.post(constants.KCURLS.VERIFYCAPTCHA, verify, function(res) {
        if (res.succeeded) {
          ...
          that.$api.post(constants.KCURLS.CHANGEPASSWORD, data, function(res1) {
            if (res1.succeeded) {
